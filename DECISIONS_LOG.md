...
### D-020 - Navigasi in-app "se-powerful Google Maps" tanpa SDK + ganti tile OpenFreeMap → CARTO
- **Keputusan:** Mengimplementasikan navigasi penuh di dalam app tanpa Google Maps SDK (permintaan user 08 Aug 2026, scope A+B+C+D): (a) **turn-by-turn** — `osrmClient` kirim `steps=true`, `orsClient` kirim `instructions:true, maneuvers:true`, parser step → `RouteStep[]` (`maneuver.location`, `maneuver.type/modifier`, `name`, `way_points`), `TurnByTurnHud` banner manuver besar (panah dirotasi, label Bahasa Indonesia, nama jalan, jarak ke manuver via `findUpcomingStep`, getar <120m via haptics); (b) **auto re-route** — saat off-route, `Route.tsx` hitung ulang rute dari posisi kurir ke stop aktif (sekali/episode, min 60s) dan tulis via `queryClient.setQueryData`; (c) **map-matching** — `useRoadMatchedLocation` proyeksikan marker ke leg aktif ("menempel di jalan"), saat off-route pakai Valhalla `trace_route` (`shape_match: map_snap`, decode polyline6); (d) deep-link `google.navigation:` tetap dipertahankan sebagai cadangan. Juga: ganti basemap **OpenFreeMap → CARTO** (`basemaps.cartocdn.com/gl/voyager-gl-style` / `positron-gl-style`) karena OpenFreeMap mengembalikan 403 untuk semua tile di WebView Capacitor (issue `hyperknot/openfreemap#114`), tambah `valhalla1.openstreetmap.de` ke `allowNavigation`.
- **Alasan:** user minta navigasi sekuat Google Maps tanpa biaya SDK; verifikasi empiris membuktikan OpenFreeMap 403 dan CARTO 200 lintas-origin (style, raster, vector `.mvt`).
- **Trade-off:** ORS/OSRM berbahasa Inggris untuk instruksi mentah (label HUD dibuat manual Bahasa Indonesia); Valhalla public best-effort (fallback ke proyeksi leg bila gagal); navigasi in-app tanpa suara/voice guidance.
- **Sumber:** keputusan user (scope A+B+C+D); COURIER_MAP_ROUTING_UPGRADE_BLUEPRINT §6.1-6.3, §8.
